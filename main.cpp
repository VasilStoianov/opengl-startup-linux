...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(side, 0.0f, 0.5f));
        const float radius = 10.0f;
        float camX = cos(side) * radius;
        float camZ = sin(side) * radius;

        view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
        projection = glm::perspective(glm::radians(fov), 800.0f / 600.0f, 0.1f, 100.0f);
        // trans = glm::rotate(trans, (float)glfwGetTim e(), glm::vec3(1.0f, 1.0f, 0.0f));
